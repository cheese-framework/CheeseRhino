    ...
    private static class SimpleShellConsole extends ShellConsole {
        ...
        SimpleShellConsole(InputStream in, PrintStream ps, Charset cs) {
            ...
            if ("Dalvik".equals(System.getProperty("java.vm.name"))){
                if (isSdkVersionAtLeast("TIRAMISU")) {
                    this.out = new PrintWriter(ps, false, StandardCharsets.UTF_8);
                } else {
                    this.out = new PrintWriter(ps, false);
                }
            }else{
                this.out = new PrintWriter(ps, false, StandardCharsets.UTF_8);
            }
        ...
        private static boolean isSdkVersionAtLeast(String versionFieldName) {
            try {
                Class<?> versionClass = Class.forName("android.os.Build$VERSION");
                java.lang.reflect.Field sdkIntField = versionClass.getField("SDK_INT");
                int sdkInt = (Integer) sdkIntField.get(null); // 获取当前设备的 SDK 版本号
                Class<?> versionCodesClass = Class.forName("android.os.Build$VERSION_CODES");
                java.lang.reflect.Field versionField = versionCodesClass.getField(versionFieldName);
                int versionCode = (Integer) versionField.get(null); // 获取指定版本的版本号
                return sdkInt >= versionCode;
            } catch (Exception e) {
                return false;
            }
        }
